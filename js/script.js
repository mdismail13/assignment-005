document
  ...
  const copyBtn = document.getElementsByClassName('fa-copy');
  ...
  const callBtn = document.getElementsByClassName('fa-phone');

  for(const btn of callBtn){
    btn.addEventListener('click', function(event){
        const serviceName = event.target.parentNode.parentNode.parentNode.childNodes[3].innerText;
        const serviceNumber = event.target.parentNode.parentNode.parentNode.childNodes[7].childNodes[1].innerText;
        const coin = Number(document.getElementById('coin').innerText);
        if(coin < 20){
            alert("❌ You don't have sufficient coins. Minimum 20 coins required to call😥")
            return;
